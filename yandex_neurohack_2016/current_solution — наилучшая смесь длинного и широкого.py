...
class DelayedRLSPredictor:
	...
	def tree_predict(self):
		...
		#predictor = [regressors[- self._M - self._delay - 1: - self._delay - 1].flatten()]
		#prediction = self.ridge.predict([predictor]	)[0]
		prediction = self.tree.predict([predictor])[0]
		#print(type(prediction))
		return prediction
...
rls = DelayedRLSPredictor(n_channels=N_CHANNELS, M=20, lambda_=0.9996, delta=0.01, delay=SKIP_SIZE, mu=1)
...
#rls.total_update_tree()
pr0 = rls.predict_linear()
pr1= rls.predict_linear2()
#pr1 = rls.ridge_predict()
w1 = 0.5
prediction = pr0 * (1-w1) + pr1 * w1
#prediction = rls.tree_predict()
